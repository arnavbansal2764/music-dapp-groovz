...
contract Groovz {
    ...
    function getLikedMusicByUser() external view returns (MusicToken[] memory) {
        ...
        MusicToken[] memory userLikedMusic = new MusicToken[](
            likedTokenIds.length
        );
    ...
    function unlikeMusic(uint256 tokenId) external {
        ...
        if (indexToRemove != likedTokenIds.length - 1) {
            likedTokenIds[indexToRemove] = likedTokenIds[
                likedTokenIds.length - 1
            ];
        }
        likedTokenIds.pop();
    ...
    function getMyBoughtMusics() external view returns (MusicToken[] memory) {
        ...
        for (uint256 i = 1; i < nextTokenId; i++) {
            if (
                musicTokens[i].owner == msg.sender &&
                musicTokens[i].author != msg.sender
            ) {
                count++;
            }
        }
